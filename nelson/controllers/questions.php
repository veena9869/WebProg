...
function view() {
	...
	while ($result = mysqli_fetch_array($query)) {
		...
		$query_nest = mysqli_query($db,$sql_nest);
...
EOD;
    ...
    $js = '<script>
    var qualified_path = "'.$_SERVER['HTTP_HOST'].$basePathNS.'";';
	...
	function vote(elem,type,voted) {
        ...
        var url = 'http://'+qualified_path + basePath + "/" + type + "/vote";
        var params = {
            question_id: id,
            vote: voted
        };
...
function index() {
	...
	if (!empty($_GET['type'])) {
		...
		if (sanitize($_GET['type'],"string") == "unanswered") {
			...
			$conditionspost .= " questions.answers = 0 and questions.kb = 0 and ";
