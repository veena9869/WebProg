...
function authenticate($force = 0) {
    ...
    $db = db();
		$sql = ("select id,name,points from users where name = '".escape($_SESSION['name'])."' and password = '".escape($_SESSION['password'])."'");
...
function db() {
	$dbh = mysqli_connect(SERVERNAME,DBUSERNAME,DBPASSWORD,DBNAME);
	return $dbh;
}
